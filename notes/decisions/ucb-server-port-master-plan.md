...
> **⚙️ ДЕТЕКТОР — БОЛЬШОЙ ЗАХОД через workflow+ревью (2026-06-04, commit `81eace1`).** Доделаны 3 трека. **Открытие при граунд-трусе:** бэкенд детектора уже был почти полон — worker гоняет recurring sweep (`AnomalyDetectorService.scanAll`), routes зарегистрированы, post_port_checks (golden_case_drift/unmatched, lp_uncovered_nearzero, pnl_impossible_negative) + ack/resolve (`PATCH /admin/golden/anomalies/:id`) + promote (`POST .../promote`) СУЩЕСТВОВАЛИ. Реальные дыры = новые проверки + per-row UI-действия.
> - **Новые проверки (4):** `swap_movement_imbalance` (pre-port, warn — свопы где \|out−in\|/max>20%; pure `findSwapMovementImbalance` + `SWAP_IMBALANCE_DEFAULTS` + SQL-чекер в admin-tech-audit/runAll; ORDER BY tie-break both-sided перед one-side-zero=100%, иначе непрайснутые ноги вытесняют actionable из LIMIT 50). `fee_apr_without_fee` (post-port, error — feeApr>0 при fee≈$0, stale Krystal POS-004/005). `stable_avgprice_off` (post-port, info — стейбл avgBuyPrice≠$1, EUR-стейблы законны→info, один finding/позицию с detail.tokens). `cost_basis_from_spot` (post-port, warn — Σ fallbackUsd >$100 или >50% startUsd, неполный провенанс). Расширены `CanonicalPosition`/`POST_PORT_THRESHOLDS`/`adaptPositions`.
> - **UI (C9 частично):** `AnomaliesPage` уже рендерил инбокс (фильтр статуса, скан) — добавлены per-row «Принять»/«Закрыть»/«В эталон» (PromoteDialog: label/sourceOfTruth/expectedStartUsd/note) + хуки `useResolveAnomaly`/`usePromoteAnomaly`.
> - **Ревью поймало блокер:** фронт ack/resolve слал PATCH без `note`, а бэкенд `resolveBody.note` = `.nullable()` БЕЗ `.optional()` → zod 400. Фикс: `note: null` явно + сужен `ResolveAnomalyBody` тип. + minor: `Number()`-обёртка feeAprLifetime, swap ORDER BY tie-break.
> - **Гейты:** api anomaly **55/55** (registry +6, post_port +11), api tsc baseline 36, web tsc baseline 251 (0 новых), vite build ✓.
> - **Осталось по детектору:** prod-валидация порогов новых post-port чеков на реальных `ucb_shadow_results` (signal/noise); промоут-петля e2e (резолв→golden→replay); остаток C5b-чеков (lp_krystal_divergence и др. — нужен Krystal server-side / post-merge).
