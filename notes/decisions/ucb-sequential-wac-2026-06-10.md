...
## TODO (зафиксировано owner 2026-06-10)

- **Fluid ETH Δ $207.68 (0.58%)** — реальное микро-расхождение display-walker
  (`position_lot_cost_basis`) vs канонический граф (`cross_protocol`) на
  сложной ETH-цепочке (клеймы 31.01 / фиат 20.04 / ETH-оплата GM 0x450b).
  Висит честным error-флагом `tracker_divergence` в «Аномалиях» (после
  asset-level доработки чека это единственный оставшийся сигнал). Разбор —
  probe-методом (`engine-fluid-probe.mts`), отдельным заходом.
- **Receipt-реестр**: при подключении нового receipt-протокола проверять
  распознавание (RECEIPT_CONTRACTS whitelist / символ-паттерны) — без него
  per-token изоляция не включится. Кандидат на детектор-чек
  «receipt-подобный токен вне реестра».
- Morpho golden — после отдельного GLV-трейса.
