...
import { supplyAmountsHash } from "./supply_hash.js";
import { buildCostBasisTracker, CostBasisTracker } from "./cost_basis_tracker.js";
import { buildLotTrackerFromOps } from "./lots/build.js";
import { PerWalletLotTrackerView } from "./lots/compat.js";
...
import type { LiveSnapshot, LiveProtocolPosition } from "./live.js";
import type {
  ClassifiedOp,
  ProtocolCategory,
  ProtocolInfo,
...
import type { SavedWallet } from "./wallet.js";
...
/**
 * Канонизация символа ЗАЛОГА для receipt-less протоколов (Morpho Blue / Drift /
 * Adrena): WETH→ETH + generic strip-w-prefix (`wSPYx`→`SPYX`), чтобы on-chain
 * wrapped-символ в реестре (`wSPYx`) сматчился с DeBank-составом (`SPYx`).
 * Зеркало `cost_basis_tracker.normalizeSymbol` (owner-решение 2026-05-31:
 * wrapped/unwrapped = один актив для POS-004 wSPYx↔SPYx).
 *
 * Используется ТОЛЬКО в receipt-less per-market scoping — общий `normalizeSymbol`
 * НЕ трогаем (расширение w-map'ой может сдвинуть lot-группировку, см.
 * wrapped_symbols.ts). Strip только `w` + ОДНА uppercase-буква (wstETH/wbETH
 * с lowercase-префиксом >1 не затрагиваются).
 */
function canonicalCollateralSymbol(s: string): string {
  const u = s.toUpperCase();
  if (u === "WETH") return "ETH";
  if (
    s.length > 1 &&
...
interface BuildOptions {
  ...
  /**
   * Lot-консумация методология (FIFO/LIFO/WAC/HIFO) от page-toggle `Lot:`.
   * Применяется к non-stable supply-token cost basis (lending/yield), где
   * startUsd lot-traced. Default WAC. Для LP startUsd идёт из Krystal —
   * methodology-independent (toggle на них не влияет).
   */
  methodology?: import("./lots/types.js").LotMethodology;
  /**
  ...
  v3MintPoolPrices?: Map<
    string,
    {
      ...
      exactAmount0?: string;
      exactAmount1?: string;
      /** Mint block (diagnostics only). A0: surfaced under strict typecheck. */
      blockNumber?: number;
    }
  >;
  /**
   ...
   * position summary.
   */
  lotsByWallet?: Map<string, import("./lots/lot_tracker.js").LotTracker>;
  /**
...
function currentCostBasisForPosition(
  ...
  histPrices?: Map<string, number>,
  /**
   * Receipt-less per-market scoping (Morpho Blue несколько рынков на кошельке):
   * символ ЗАЛОГА именно этой позиции (из `lp.supply`). Если передан — берём его
   * как `collateralSymbol` вместо эвристики «первый non-stable OUT» (которая на
   * shared ops-листе давала ВСЕМ рынкам самый ранний залог). Канонизируется
   * через `canonicalCollateralSymbol` (wSPYx↔SPYx). POS-004.
   */
  ...
  if (receiptLessMode) {
    // Приоритет: явный hint залога этой позиции (per-market scoping POS-004).
    // Без hint — эвристика «первый non-stable OUT» (single-market backward-compat).
    if (collateralHint) {
      collateralSymbol = canonicalCollateralSymbol(collateralHint);
    } else {
      for (const op of sorted) {
        for (const m of op.movement) {
          if (m.direction !== "out" || m.amount <= 0) continue;
          ...
          if (
            ...
            continue; // gas
          collateralSymbol = canonicalCollateralSymbol(m.symbol);
          break;
        }
  ...
  for (const op of sorted) {
    ...
    if (receiptLessMode && collateralSymbol) {
      for (const m of op.movement) {
        if (m.direction !== "out" || m.amount <= 0) continue;
        if (canonicalCollateralSymbol(m.symbol) !== collateralSymbol) continue;
...
function buildOne(
  ...
  /**
   * Lot-консумация методология (FIFO/LIFO/WAC) от page-toggle. Применяется к
   * non-stable supply-token startUsd через getPositionLotCostBasis. Default WAC.
   */
  methodology?: import("./lots/types.js").LotMethodology,
): OpenPosition | null {
  ...
  const supplyTokens: OpenPositionToken[] = lp.supply.map((s) => {
    ...
    const lotCb = isStable
      ...
      : getPositionLotCostBasis({
          ...
          methodology: methodology ?? "WAC",
          useNetSuppliedAmount: true,
          ...(overrideByHash && { costBasisOverrideByHash: overrideByHash }),
          histPrices,
        });
    // M6 РЕВИЗИЯ (owner 2026-05-31): частичное покрытие истории покупок.
    // covered amount → trusted lot-cost (FIFO/LIFO/WAC); uncovered остаток
    // (supplied − covered) → цена на момент ОТКРЫТИЯ позиции (supply-op on-chain
    // price = cycleDeposit.usd/amount), НЕ $0 и НЕ suppress. uncovered-доля
    // помечается через `fallbackUsd` (priced-not-trusted) → UI-провенанс
    // «оценено по входу». Так нет дыр $0, PnL не раздувается, startAmount =
    // полный supplied. trustedPct = covered/supplied (репортинг, не гасит).
    const lotConsumed: { amount: number; usd: number; fallbackUsd: number } =
      isStable || !lotCb
        ? { amount: 0, usd: 0, fallbackUsd: 0 }
        : (() => {
            const covered = lotCb.totalAmountSupplied;
            const uncovered = Math.max(0, lotCb.uncoveredAmount);
            const openPrice =
              cycleDeposit.amount > 0
                ? cycleDeposit.usd / cycleDeposit.amount
                : s.amount > 0
                  ? s.usd / s.amount
                  : 0;
            const uncoveredUsd = uncovered * openPrice;
            return {
              amount: covered + uncovered, // полный net-supplied (covered + uncovered)
              usd: lotCb.totalCostUsd + uncoveredUsd, // trusted cost + open-priced остаток
              fallbackUsd: uncoveredUsd, // priced-not-trusted доля (провенанс-ярлык)
            };
          })();
  ...
  // Receipt-less per-market scoping (Morpho Blue: несколько рынков на одном
  // кошельке). Залог ИМЕННО этой позиции = доминирующий live supply-token
  // (per-market из DeBank). Передаём как hint в cost-basis + openedInTokens,
  ...
  const isReceiptLess = isReceiptLessPos;
  ...
  const matchingOps = ops.filter((op) => {
    ...
    if (
      ...
      return false;
    // Receipt-less per-market scoping (Morpho Blue): lpTokenId = singleton-
    // контракт → opMatchesLpMarket бесполезен, и без скоупа op'ы ВСЕХ рынков
    // (wSPYx/PT-apxUSD/PT-apyUSD) текли в openedInTokens каждой позиции
    // (PT — isProtocolToken → Pass 1 перехватывал их у SPYx-позиции). Скоупим
    // по залогу ИМЕННО этой позиции: op должен двигать его OUT. POS-004.
    if (isReceiptLess && collateralCanon) {
      const movesCollateral = op.movement.some(
        (m) =>
          m.direction === "out" &&
          m.amount > 0 &&
          canonicalCollateralSymbol(m.symbol) === collateralCanon,
      );
      if (!movesCollateral) return false;
    }
    return true;
  });
  ...
  const openedInTokens = Array.from(openedInMap.entries()).map(([symbol, v]) => {
    ...
    const cleanTid = v.tokenId
      ? v.tokenId
          ...
          .replace(/:[a-z][a-z0-9_-]+$/i, "")
      : undefined;
    // Receipt-less: показываем символ залога как в составе (DeBank `SPYx`),
    // а не сырой on-chain wrapped (`wSPYx`) — owner-решение wSPYx↔SPYx (POS-004).
    const displaySymbol =
      collateralHint &&
      collateralCanon &&
      canonicalCollateralSymbol(symbol) === collateralCanon
        ? collateralHint
        : symbol;
    return {
  ...
  return {
    ...
    chain: lp.chain,
    protocol: { id: lp.protocolId, name: lp.protocolName, category: lp.category as ProtocolCategory },
    ...
    survivors.forEach((cand, idx) => {
      ...
      out.push({
        ...
        protocol: {
          ...
          category: cand.protocolCategory as ProtocolCategory,
        },
        ...
        feesClaimedUsd,
        feesLifetimeUsd,
        // A0: inferred-history positions have no per-event claim history —
        // empty array is the correct representation (the normal build path
        // sets a populated array). Previously omitted (undefined at runtime),
        // never read for inferred positions; surfaced under strict typecheck.
        feesClaimedHistory: [],
        feeApr: null,
        feeAprLifetime,
        feesByToken: [],
