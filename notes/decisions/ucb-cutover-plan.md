...
## 🔄 ОБНОВЛЕНИЕ 2026-06-13 — cutover частично уже реализован
С момента написания плана (2026-06-08) добавлено (ветка `capital-summary`):
- **`ucbServerOnly`** (`capflow.feature.ucbServerOnly`, commit `044dc65`) — режим, где
  браузер НЕ считает вовсе (сильнее `ucbServerCanonical`). Это финал шага 6 (снос
  браузерной оркестрации), но **через флаг**, не удалением кода. Прод: **OFF**.
- **Сервер = источник** (`da1061f`): фронт принимает серверные позиции при
  `ucbServerCanonical` ON; бейдж «расчёт: сервер/браузер (причина)» делает любой
  fallback видимым (сверка больше не вслепую).
- **Авто-recompute** (`899875e`/`f1f89f5`): при `no_shadow`/`not_served`/
  `methodology_mismatch`/`stale` клиент дёргает `POST /ucb/recompute` → shadow
  создаётся на лету. ⚠ При включении `ucbServerOnly` БЕЗ предварительного shadow по
  всем — непосчитанные аккаунты сначала покажут пусто, затем подтянут (класс moximko
  2026-06-13). Порядок строгий: **сначала shadow по ВСЕМ (шаг 1), потом флипы**.
- **Добавление адреса → refresh+shadow** (`745502b`): новый аккаунт получает shadow
  сразу при подключении (закрывает «recurring только на буте воркера»).
- **Паритет сервер⊇браузер** (`2a3b245`): 4 последние browser-only V3-операции
  перенесены в движок → шаг 6 технически разблокирован.

Остаток (не изменилось): **клиентский shadow-diff POST всё ещё надо включить** из
`use_computed_positions` (шаг 2 ниже) — без него `diff_summary` пуст. Но с
`ucbServerOnly` бейдж уже честно показывает источник, так что слепоты меньше.
