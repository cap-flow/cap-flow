...
>
> ⏸️ **B6 SLICE 2 — UI adoption in `useComputedPositions` (DESIGNED, NOT built — deferred to a focused session).** Plan: behind `capflow.feature.ucbServerCanonical` (reactive `useResolvedFeatureFlag`), react-query `GET /ucb/positions`; adopt server positions ONLY when `resp.serve && resp.lotMethodology === user's lotMethodology` (← **methodology-match guard**: cleanly avoids serving FIFO-computed positions to a LIFO user WITHOUT needing server-side per-user methodology persistence — the heavier B6 follow-up becomes optional, only widening flip coverage). Else fall back to the existing client recompute (permanent, R16). Public `ComputedPositions` shape unchanged → zero page churn; keep `positionsRaw` for diagnostics. **⚠ Integration subtleties found in survey (why this needs a focused session, not a long-session tail):** (1) `useComputedPositions` has NO accountId in scope — endpoint is `/accounts/:id/...`; the account UUID must be derived (composite wallet id `api:<accountUuid>:<walletUuid>`), and the hook may hold wallets across accounts; (2) **wallet-set match** — server shadow = the FULL account, client `loadedList` = whatever's loaded in the browser (possibly a subset) → adopt only when sets match, else fall back; (3) **wire serialization** — served `OpenPosition[]` crossed JSON (Date/bigint fields) → revive before use. Value is gated on the merge→prod-soak anyway (flip needs ≥2wk zero diffs), so no rush to land it dormant. **Test (when built):** unit serve-vs-fallback (done in slice 1 core); hook test: flag ON+fresh+method-match → server; stale/error/OFF/method-mismatch/wallet-set-mismatch → client; golden integration served==recomputed; kill API → graceful fallback.
