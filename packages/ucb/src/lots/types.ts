...
export interface ConsumeOptions {
  ...
  amount: number;
  /**
   * A0: relaxed to optional to match actual call sites (the fresh-tracker
   * replay in `position_lot_cost_basis` omits it → `consumes[].time` is
   * `undefined`, preserved verbatim). Required-but-omitted only ever
   * "compiled" in web because web never ran strict `tsc` on this code.
   */
  consumedAt?: number;
  walletId: string;
}
