...
export interface SuppliedLotConsumption {
  ...
  costUsd: number;
  acquiredVia: string;
  /** A3.6: the blockchain tx that created the consumed lot (cause→effect link). */
  sourceHash?: string;
  /** Lot's own asset symbol (may differ from the supplied symbol via wrapping). */
  symbol?: string;
  chain?: string;
}
...
export function getPositionLotCostBasis(
  ...
  const consumedLots: SuppliedLotConsumption[] = result.consumed.map((c) => ({
    ...
    ...(c.lot.sourceHash ? { sourceHash: c.lot.sourceHash } : {}),
    symbol: c.lot.symbol,
    chain: c.lot.chain,
  }));
