...
export interface FiatPurchaseAnnotation {
  ...
  /**
   * **Зафиксированный** $-эквивалент покупки на момент входа. Это «стартовый
   * капитал» в долларах — фиксированная величина, которая НЕ пересчитывается
   * по текущему курсу (мы реально купили эти доллары за `fiatAmount` фиата).
   *
   * Если не задан (legacy-пометки без USD) — расчёт стартового капитала в $
   * падает на конвертацию `fiatAmount` по текущему курсу ЦБ (см. metrics.ts),
   * что и даёт нежелательную «курсовую» переоценку.
   */
  usdAmount?: number;
  /**
   * Курс фиат/USD (сколько единиц фиата за $1), по которому зафиксирован
   * `usdAmount`. Хранится для прозрачности/отображения; в расчётах не
   * используется напрямую — источником истины является `usdAmount`.
   */
  usdRate?: number;
