...
export class LotTracker {
  ...
  consume(opts: ConsumeOptions): ConsumeResult {
    ...
    for (const lot of order) {
      ...
      // A0: `consumedAt` is optional (see ConsumeOptions) — preserve the
      // pre-existing runtime where it may be undefined; `as number` keeps the
      // `consumes[].time` shape unchanged without altering behaviour.
      lot.consumes.push({ time: opts.consumedAt as number, amount: take });
      remaining -= take;
      totalCost += costPart;
      totalAmount += take;
    }
